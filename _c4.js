...
const PROFILE = {
  name:"Kathleen", planName:"UnitedHealthcare", memberId:"•••• 789",
  visitCost:"a $0 copay", glp1Pathway:"bridge", bridgeCopay:"$50 a month",
  provider:"Dr. Nguyen", program:"Bold doctor-supervised GLP-1 weight loss program"
};
let MODE = "known";
...
function showPage(id){
  ["page-inbox","page-chat","page-simple"].forEach(p=>{ document.getElementById(p).hidden = (p!==id); });
}
...
document.getElementById("chat-back").onclick = ()=>showPage("page-inbox");
...
document.getElementById("seg").addEventListener("click", e=>{
  ...
  MODE=btn.dataset.mode; chatStarted=false; renderInbox(); // refresh previews; chat re-greets on next open
});

renderInbox(); showPage("page-inbox");
