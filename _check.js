...
const PROFILE = {
  ...
  provider:    "Ali N.",               // matched Care Coordinator
  apptWhen:    "Monday, Nov 3 · 9:00 AM PT",
...
const READY = { label:"Get ready for my visit", go:"visit" };
...
const NODES = {
  ...
  visit(){
    if(MODE==="known"){
      return {
        text:[`You already have your intake call booked — a 15-minute call with <b>${PROFILE.provider}</b> on <b>${PROFILE.apptWhen}</b>, at no cost.`,
              "Here's how it works so you feel ready:"],
        steps:[
          "We'll call you at your scheduled time — keep your phone nearby.",
          "Ali will confirm your coverage and cost, then help you book your provider appointment.",
          "Your first provider visit is a 45-minute video call from home — no travel, no waiting room."
        ],
        callout:"Want a quick 2-minute Zoom test beforehand? I can walk you through it, one step at a time.",
        sugg:[{label:"Do a Zoom test", go:"zoomTest"},{label:"What should I have ready?", go:"prep"},{label:"Reschedule my call", go:"reschedule"}]
      };
    }
    return {
      ...
      sugg:[{label:"Do a Zoom test", go:"zoomTest"},{label:"What should I have ready?", go:"prep"}]
    };
  },
  zoomTest(){
    return {
  ...
  reschedule(){
    return {
      text:[`No problem — I can help you move your call with ${PROFILE.provider}.`,
            "Your Care Coordinator handles scheduling changes. Call or text 424-577-5266 and they'll find a time that works for you."],
      sugg:[{label:"Ask something else", go:"menu"}]
    };
  },
...
const MENU_PILLS = [
  ...
  {label:"Get ready for my visit", go:"visit"},
  {label:"What is Bold, and what's next?", go:"program"}
];
...
function routeText(t){
  ...
  if(has("zoom","video","visit","appointment","call","download","telehealth","virtual","in person","in-person","office","reschedule","ready")) return "visit";
...
function knownCardHTML(){
  return `<div class="known-card">
    ...
    <div class="kgrid">
      ...
      <div class="krow"><span class="kk">Intake call</span><span class="kv">${PROFILE.provider} · ${PROFILE.apptWhen}</span></div>
    </div>
...
function renderQuick(){
  quick.innerHTML = "";
  MENU_PILLS.forEach(p=>{
    const b = document.createElement("button");
    b.className = "pill"; b.textContent = p.label; b.onclick = ()=>choose(p);
    quick.appendChild(b);
  });
}
function greet(){
